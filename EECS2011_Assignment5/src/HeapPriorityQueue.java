...
public class HeapPriorityQueue<K,V> extends AbstractPriorityQueue<K,V> {
    ...
    protected void downheapRec( int j ) {
        ...
        if(hasLeft(j)){
            ...
            if (compare(heap.get(smallChildIndex), heap.get(j)) >= 0) return;
            swap(j, smallChildIndex);
            j = smallChildIndex;
            downheapRec(j);
        }
    ...
    public int updateKey( K currentKey, K newKey ) {
        ...
        int exist = find(1,currentKey);
        if(exist == -1) return -1;
        Entry<K,V> n = heap.get(exist);
        n = Entry((Integer) currentKey, (Integer) heap.get(exist).getValue());
        heap.set(exist,n);
        return exist;    // replace this line with your code

    }

    public int find(int k, K keyToFind){
        if(heap.get(k).getKey() == keyToFind){
            return k;
        }
        if(hasLeft(k)){
            k = left(k);
